...
# by G Oldford 2024
# processes NEMO daily NC to daily means into annual NC files
# the daily files (as annual NC) are converted by another script to ASC
# the annual files are used in calculations of PAR and then converted to ASC (annual files faster than month)
# command prompt example: 'ksh 1_NEMO_[etc].ksh'
...
pathout2="/NEMO_annualdaily_NC/"
#pathout3="/NEMO_monthly_NC/"
pathout_da=$pathout1""$modelrun""$pathout3
...
# LEFT IT HERE 2024-05-20
# need to first concat monthy to annual b/c then the subsequent operations faster (not done)
...
for y in $yrs ; do
  ...
  for m in $mos ; do 
    ...
    for grid in "${grids3D_to_2D[@]}" ; do
      ...
      echo ${pathin}SalishSea1500-RUN${modelrun}_1d_grid_${grid}_y${y}m${m}.nc ;
      #echo ${pathout_in}SalishSea1500-RUN${modelrun}_DailyMean_grid_${grid}_y${y}.nc
      cdo sellevel,0.500000298,1.5000031,2.50001144,3.50003052,4.50007057,5.50015068,6.50031042,7.50062323,8.50123596,9.50243282,10.5047655 {pathin}SalishSea1500-RUN${modelrun}_1d_grid_${grid}_y${y}m${m}.nc ${pathtemp}RUN${modelrun}_DailyMean_grid_${grid}_top10m_y${y}m${m}.nc ; 
      
      #cdo vertmean ${pathtemp}RUN${modelrun}_DailyMean_grid_${grid}_top10m_y${y}m${m}.nc ${pathout_da}SalishSea1500-RUN${modelrun}_DailyMean_grid_${grid}_avgtop10m_y${y}m${m}.nc ;   
  ...
  #for grid in "${grids3D_to_2D[@]}" ; do
   # ncrcat ${pathout_da}SalishSea1500-RUN${modelrun}_DailyMean_grid_${grid}_avgtop10m_y${y}* ${pathout_yr}SalishSea1500-RUN${modelrun}_MonthlyMean_grid_${grid}_avgtop10m_y${y}.nc ;
  #done                                                                                                                    
#done
