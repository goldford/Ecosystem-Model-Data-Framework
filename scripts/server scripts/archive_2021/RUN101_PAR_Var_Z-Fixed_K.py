...
mixingSubDir = "DATA/SS1500-RUN202/CDF/" # added GO (files created from .ksh)
...
for var in lstVars:
    for iyear in range(startYear, endyear):
        ...
        #lightfullpath = os.path.join(lightDir,LightSubDir, fnTempLight.format(iyear))
        ...
        for imon in range(1,13):
            ...
            for iday in  range(1,ndays[1]+1):
                ...
                #salinity = getSalintyMatrix(iyear-1,imon,iday,salinitydepthlayer)
                #XXXXXXXXXXXXXXXXXXXXXXXXXXXXX

                if(bnewMonth):
                    dailymeans = np.empty([ndays[1],varsMixing.shape[1],varsMixing.shape[2]])
                    bnewMonth = False
