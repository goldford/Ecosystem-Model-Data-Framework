...
# Purpose: Process daily means from NEMO out to monthly means, max, mins, and sd(?)
...
#
# graham enviro prep - command line: 
# module load StdEnv/2020
# module load scipy-stack/2020b


from GO_helpers import buildSortableString, read_sdomains
...
nc_p = project_p + "DATA/SS1500-RUN216/NEMO_monthly_NC/"
tmask_p = "/project/6006412/goldford/data/mesh_mask/"
tmask_f = "mesh_mask_20210406.nc"
out_p = project_p + f"DATA/SS1500-RUN{modelrun}/NEMO_monthly_NC_pacea/"
frmask_p = project_p + "SCRIPTS/"
frmask_f = "FraserRiverMaskPnts.yml"
frmask_fullp = os.path.join(frmask_p, frmask_f)
...
depth_groups = {"surface":{"min_t_idx":0, "max_t_idx":3, "min_z":0, "max_z":4},
                "5m":{"min_t_idx":5, "max_t_idx":5, "min_z":5, "max_z":5},
...
depth_groups = {"5m":{"min_t_idx":5, "max_t_idx":5, "min_z":5, "max_z":5} } # for jan on 2024-10-29
variables = {'votemper':"temperature", 'vosaline':"salinity"}
...
# unsure why tmask is exporting zeros instead of nodata but this works to fix
...
for i in range(tmask.shape[2]):
    for j in range(tmask.shape[3]):
        ...
        if npmax == 0:
            ...
            bottom_idx = np.where(tmask[0, :, i, j])[0][-1] # needs to be tmask
    ...
    for var, var_name in variables.items():
        ...
        for dep_bin, dep_rng in depth_groups.items():
            ...
            if dep_bin == 'bottom':
                ...
                output_filename = f'hotssea_{start_yr}to{end_yr}_{min_z}m_{var_name}_{stat.lower()}.nc'
            else:
		...
		# apply fraser river mask
            ...
            # crosscheck
            #with nc.Dataset(os.path.join(out_p, output_filename)) as dat:
