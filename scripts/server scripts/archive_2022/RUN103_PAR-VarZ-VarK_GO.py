...
# created by GO + JB 2021, last edited by GO 2022-01-13

# purpose: prep files for ECOSPACE diagnostic run 103 (variable Z, variable K)
#         - RUN103 - linear salinity->turbidity (i.e. K), mixing layer depth Z
...
# 2/ Salinity - can be either vertical averages over chosen levels or from a specific depth level, post-processed from NEMO in annual NC files "xxx_MonthlyVertMean_grid_T_singledepth4m_1980.nc"
...
#                Determined K linear regression params (revision)
#                Determined salinity taken from 4m will be used
#xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
...
plumeSubDir = "DATA/"
mixingSubDir = "DATA/SS1500-RUN202/NEMO_annualNC" 
salinitySubDir = "DATA/SS1500-RUN202/NEMO_annualNC"
outputsSubDir ="DATA/SS1500-RUN202/ECOSPACE_in_RUN103"
...
dctVarnameToEwEName = {varNamesMixing:'LightAttenuation_PAR-VarZ-VarK'}
...
K = 0.05 #baseline K (clear ocean water)
#xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx

salinitydepthlayer = 4 #GO 2021-12-22 unused now
...
for var in lstVars:
    for iyear in range(startYear, endyear):
        ...
        for imon in range(1,13):
            ...
            # GO2021-12-22 - to do: get rid of these two loops
            for irow in range(varsSalinity.shape[1]):
              for icol in range(varsSalinity.shape[2]):
                
                mixingZ = MixingMon[irow,icol]
                sal =  SalinityMon[irow,icol]  # for K
                light =  LightMon[irow,icol]

                if (sal != 0): #GO-2021-12-22 - to-do: should use land / water mask as check instead
            ...
            EwEName = "RUN103-Kfromsal_singlelev4m"
