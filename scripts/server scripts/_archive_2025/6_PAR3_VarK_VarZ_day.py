...
# created by GO, last edited by GO 2024-05-21

# purpose: prep files for ECOSPACE
#          input: daily mean light from RDRS climate model, atten coef (K) from mixing depth Z and salin
...
avg_salin = False
if avg_salin == True: 
  PAR_code = PAR_code + "_Sal{}mAvg".format(salinitydepthlayer)
else:
  PAR_code = PAR_code + "_Sal{}m".format(salinitydepthlayer)
...
outputsSubDir ="DATA/SS1500-RUN{}/ECOSPACE_in_daily{}".format(NEMO_run, PAR_code)
    ...
    for iyear in range(startYear, endyear+1):
        ...
        for imon in range(startMonth,13):
            ...
            for iday in range(1,num_days+1):
                ...
                if avg_salin == True:
                  SalinityDay1 = varsSalinity[iday-1,0:salinitydepthlayer,:,:]
                  SalinityDay = np.ma.average(SalinityDay1, axis = 0)
                else:
                  SalinityDay = varsSalinity[iday-1,salinitydepthlayer,:,:]
                  print(SalinityDay.shape)
            
                ar_inf = np.where(np.isinf(SalinityDay))
                ...
                if (os.path.isdir(savepath) != True):
                    os.mkdir(savepath)
                savefn = saveTemplate.format(EwEName,iyear,buildSortableString(imon,2),buildSortableString(iday,2))
