...
# 2024-09
# need to stratify to the closest_ecospace_time level so variability is not gauranteed to be higher
print("length before:")
...
df_simplified = pd.concat([df_numeric, df_non_numeric], axis=1)

print("length after:")
print(len(df_simplified))

df_filtered = df_simplified
...
# Calculate y-limits for each row, now col
y_limits = []
...
fig.legend(custom_lines, ['Ecospace', 'SMELT', 'Observations'], loc='center', ncol=3, bbox_to_anchor=(0.5, 0.02), fontsize=12)
