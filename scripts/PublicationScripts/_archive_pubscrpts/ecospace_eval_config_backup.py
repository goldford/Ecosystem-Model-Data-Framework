...
# Preserve the baseline total inventory (NU_FREE_INIT_GNM2 + ecopath bound),
# but force the budget to close at t0 by adjusting free_init = total_ref - bound_t0
NU_FREE_INIT_MODE = "t0_preserve_total"      # "config" | "t0_preserve_total"
NU_DEBUG_INIT_ANCHOR = False

# Apply the flux multiplier to TOTAL inventory, then re-compute free as residual
...
NU_FLUX_APPLY_MODE = "scale_total"
# If the multiplier is missing for some rows, treat missing as 1 (neutral)
NU_FLUX_MULT_FILL = 1.0

# Print the anchor diagnostics (highly recommended once)
NU_DEBUG_INIT_ANCHOR = True
...
NU_MODEL_SERIES = ["box"]     # "matched" (existing), "box" (new)
...
# Turn on/off adding Ecosim series to the figure
NU_OVERLAY_ECOSIM = True
# Path to the Ecosim nutrient output CSV (produced by your Ecosim nutrient eval script)
NU_ECOSIM_NUTRIENTS_CSV = os.path.join(EVALOUT_P, f"ecosim_{ECOSPACE_SC}_onerun_B_dates_seasons_nutrients.csv")
# Which Ecosim column to plot (must exist in NU_ECOSIM_NUTRIENTS_CSV)
# Good defaults:
#   - "model_N_free_used_gN_m2"  (if you applied a flux multiplier)
#   - "model_N_free_gN_m2"       (raw inferred free without multiplier)
#   - "model_N_bound_gN_m2"      (bound pool)
