...
# --- small env helpers for parameter sweeps ---
...
# Optional sweep helpers (set via environment variables)
BT_RUN_TAG = _env_str('BT_RUN_TAG', '').strip()
BT_OUTDIR = _env_str('BT_OUTDIR', EVALOUT_P)
BT_CACHE_OUTDIR = _env_str('BT_CACHE_OUTDIR', BT_OUTDIR)
BT_FIGS_OUTDIR = _env_str('BT_FIGS_OUTDIR', FIGS_P)
...
def _bt_tagged_filename(stem: str, ext: str) -> str:
    ext = ext.lstrip('.')
    return f"{stem}_{BT_RUN_TAG}.{ext}" if BT_RUN_TAG else f"{stem}.{ext}"

# Cache paths (shared across runs when BT_CACHE_OUTDIR is common)
BT_SAT_MASK_PF = os.path.join(BT_CACHE_OUTDIR, f"suchy_ecospace_mask_{BT_MASK_REGNM}.nc")

BT_RECOMPUTE_BLOOM_TIMING_SAT = _env_bool('BT_RECOMPUTE_BLOOM_TIMING_SAT', True)  # True to force recomputation
BT_RECOMPUTE_BLOOM_TIMING_C09 = _env_bool('BT_RECOMPUTE_BLOOM_TIMING_C09', False)

BT_START_YEAR = 1980 # analysis years (exclude spinup?)
BT_END_YEAR = 2018

BT_CREATE_MASKS = _env_bool('BT_CREATE_MASKS', True)  # True to regenerate masks
...
BT_VARS_TO_ANALYZE_SAT = _env_list('BT_VARS_TO_ANALYZE_SAT', ['PP1-DIA','PP2-NAN','PP3-PIC'])
BT_ANNUAL_AVG_METHOD_SAT = _env_str('BT_ANNUAL_AVG_METHOD_SAT', 'annual') # 'annual' or 'all' - should average bloom compared against be from all years, or just one year
...
BT_LOG_TRANSFORM_SAT = _env_bool('BT_LOG_TRANSFORM_SAT', False)  #
BT_MEAN_OR_MEDIAN_SAT = _env_str('BT_MEAN_OR_MEDIAN_SAT', 'median')  # 'median" or "mean"
