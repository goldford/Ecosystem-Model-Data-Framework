...
def _plot_overlay(obs_clima: pd.DataFrame | None, model_clima: pd.DataFrame, ecospace_climas: dict | None, *, out_png: str) -> None:
    ...
    plt.plot(x, model_clima["model_center"], label="Ecosim center")
    ...
    if ecospace_climas:
        if 'matched' in ecospace_climas and ecospace_climas['matched'] is not None and not ecospace_climas['matched'].empty:
            es = ecospace_climas['matched']
            xe = es.index.to_numpy()
            plt.fill_between(xe, es['es_q10'], es['es_q90'], alpha=0.15, label='Ecospace matched 10–90%')
            plt.plot(xe, es['es_center'], linestyle='--', label='Ecospace matched center')

        if 'box' in ecospace_climas and ecospace_climas['box'] is not None and not ecospace_climas['box'].empty:
            es = ecospace_climas['box']
            xe = es.index.to_numpy()
            plt.fill_between(xe, es['es_q10'], es['es_q90'], alpha=0.15, label='Ecospace box 10–90%')
            plt.plot(xe, es['es_center'], linestyle=':', label='Ecospace box center')
