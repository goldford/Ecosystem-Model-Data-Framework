...
#
# Output:
#    plots of data descriptors and distributions, monthly boxplots of model vs obs phytoplankton
#
import pandas as pd
import calendar
import numpy as np
...
groupby_columns = ['eventID', 'year', 'month','class','closest_ecospace_time']
...
def calculate_anomalies(df, valuefield, groupon):
    ...
    # reduce by first finding expected (mean) value at ECOSPACE time step level
    pd.set_option('display.max_columns', None)
    pd.set_option('display.max_rows', None)
    if 'level_0' not in df.columns:
        df = df.reset_index()
    # Then drop the 'eventID' column
    # print(df.head())
    print("length before")
    print(len(df))
    if 'eventID' in df.columns:
        df = df.drop(columns=['eventID'])
    df = df.groupby(['year', 'month', groupon, 'closest_ecospace_time']).mean().reset_index()
    print("length after")
    print(len(df))
    # (added 2024-09-16, experimental)
...
####################################################
######## PP1-DIA - Ecospace vs QU39 vs SSC #########
...
if include_SSC:
    ...
    positions_qu39 = months + 0.25  # Adjust positions slightly to the left
    widths = 0.2
    ...
    for month in months:
        ...
        # ecospace_data = data_by_month[month].get('PP1-DIA', [])
        # qu39_data = data_by_month[month].get('QU39', [])
        # ssc_data = data_by_month[month].get('ssc-DIA', [])
        ...
        plt.boxplot(ecospace_data, positions=[positions_ecospace[month - 1]], widths=widths, patch_artist=True,
                    ...
                    # boxprops=dict(facecolor=colors['PP1-DIA'], color='black'),
        ...
        plt.boxplot(ssc_data, positions=[positions_ssc[month - 1]], widths=widths, patch_artist=True,
                    ...
                    # boxprops=dict(facecolor=colors['ssc-DIA'], color='black'),
                    medianprops=dict(color='black'),
                    whiskerprops=dict(color='black'),
                    capprops=dict(color='black'),
                    showfliers=False)
        ...
        plt.boxplot(qu39_data, positions=[positions_qu39[month - 1]], widths=widths, patch_artist=True,
                    ...
                    # boxprops=dict(facecolor=colors['QU39'], color='black'),
                    medianprops=dict(color='black'),
                    whiskerprops=dict(color='black'),
                    capprops=dict(color='black'),
                    showfliers=False)
    ...
    # leg_codes = ['PP1-DIA', 'ssc-DIA', 'QU39']
    ...
    plt.tight_layout()
    plt.savefig(out_p + model_run + '_Diatoms_Monthly_QU39_vs_Ecospace_vs_SSC_2016_2018.png')
    plt.show()



#############################################
######## PP1-DIA - Ecospace vs QU39 #########
...
plt.tight_layout()
plt.savefig(out_p + model_run + '_Diatoms_Monthly_QU39_vs_Ecospace_2016_2018.png')
plt.show()
...
months = np.arange(1, 13)
if include_SSC:
    field_codes = ['log_PP2-NAN', 'logQU39', 'log_ssc-FLA']
    source_names = ['Ecospace', 'QU39', 'SMELT']
    colors = {'log_PP2-NAN': 'blue',  'logQU39': 'orange', 'log_ssc-FLA': 'pink'}
    leg_codes = ['log_PP2-NAN', 'logQU39', 'log_ssc-FLA']
    positions_ecospace = months - 0.25 # adjust slightly right
    positions_ssc = months
    positions_qu39 = months + 0.25  # Adjust positions slightly to the left
    widths = 0.2
...
for month in months:
    ...
    if include_SSC:
        ssc_data = data_by_month[month].get('log_ssc-FLA', [])
        # # Plot the second model
        plt.boxplot(ssc_data, positions=[positions_ssc[month - 1]], widths=widths, patch_artist=True,
                    boxprops=dict(facecolor=colors['log_ssc-FLA'], color='black'),
                    medianprops=dict(color='black'),
                    whiskerprops=dict(color='black'),
                    capprops=dict(color='black'),
                    showfliers=False)
...
plt.tight_layout()
if include_SSC:
    plt.savefig(out_p + plot_label + "_" + model_run + '_Monthly_QU39_vs_Ecospace_vs_SSC_2016_2018.png')
else:
    plt.savefig(out_p + plot_label + "_" + model_run + '_Monthly_QU39_vs_Ecospace_2016_2018.png')
plt.show()
...
# Example: for nanophytoplankton group
taxa_to_plot = dino_taxa
plot_label = 'Dinoflagellates'
...
# field_codes = ['PP1-DIA', 'ssc-DIA', 'QU39']
field_codes = ['log_PZ2-DIN', 'logQU39']
# source_names = ['Ecospace', 'SalishSeaCast', 'QU39']
source_names = ['Ecospace', 'QU39']

# Colors for each model_code
colors = {'log_PZ2-DIN': 'blue', 'logQU39': 'orange'}
...
for month in months:
    ...
    # Data for both models for the current month
    ecospace_data = data_by_month[month].get('log_PZ2-DIN', [])
    qu39_data = data_by_month[month].get('logQU39', [])

    # Plot the first model
    plt.boxplot(ecospace_data, positions=[positions_ecospace[month - 1]], widths=widths, patch_artist=True,
                boxprops=dict(facecolor=colors['log_PZ2-DIN'], color='black'),
                medianprops=dict(color='black'),
                whiskerprops=dict(color='black'),
                capprops=dict(color='black'),
                showfliers=False)
...
plt.grid(True)
plt.savefig(out_p + 'Histo_Diatoms_PP_QU39_2016_2019.png')
plt.show()
...
plt.legend(title='Taxonomic Class')
plt.grid(True)
plt.savefig(out_p + 'Histo_' + model_run + '_Diatoms_PP_SSCDIA_2016_2019.png')
plt.show()
...
plt.legend(title='Taxonomic Class')
plt.grid(True)
plt.savefig(out_p + 'Histo_Diatoms_PP_PP1DIA_2016_2019.png')
plt.show()
