...
#  - SSC model output file (e.g., SalishSeaCast_biology_2008.nc)
#  - Nemcek data of Chla conc by group (i.e., Nemcek_Supp_Data.csv)
# Output files:
#  - ECOSPACE and SSC outputs joined to the original Nemcek et al dataset
#
# Sources of Data:
...
with nc.Dataset(ecospace_file_path, 'r') as dataset:
    for index, row in nemcek_df.iterrows():
        ...
        if row_idx != -999 and col_idx != -999:
            ...
            # rounding
            pp1_dia_value = np.where(pp1_dia_value == None, np.nan, np.round(pp1_dia_value, 3))
            pp2_nan_value = np.where(pp2_nan_value == None, np.nan, np.round(pp2_nan_value, 3))
            pp3_pic_value = np.where(pp3_pic_value == None, np.nan, np.round(pp3_pic_value, 3))
            pz1_cil_value = np.where(pz1_cil_value == None, np.nan, np.round(pz1_cil_value, 3))
            pz2_din_value = np.where(pz2_din_value == None, np.nan, np.round(pz2_din_value, 3))
        ...
        ssc_ciliates = ssc_ciliates.astype(np.float64)
        ssc_ciliates = np.round(ssc_ciliates, decimals=3)

        ssc_flagellates = np.where(ssc_flagellates == None, np.nan, ssc_flagellates)
        ssc_flagellates = ssc_flagellates.astype(np.float64)
        ssc_flagellates = np.round(ssc_flagellates, decimals=3)

        ssc_diatoms = np.where(ssc_diatoms == None, np.nan, ssc_diatoms)
        ssc_diatoms = ssc_diatoms.astype(np.float64)
        ssc_diatoms = np.round(ssc_diatoms, decimals=3)
