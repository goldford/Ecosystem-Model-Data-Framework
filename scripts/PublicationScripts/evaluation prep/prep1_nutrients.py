...
# -------------------------------
# match each sample to ecospace row col
# ------------------------------
...
ecospace_path = os.path.join(ECOSPACE_MAP_P, ECOSPACE_MAP_F)

# Load Ecospace grid map
ecospace_df = pd.read_csv(ecospace_path)
ecospace_df.rename(columns={"lat": "latitude", "lon": "longitude"}, inplace=True)

# Build KDTree for fast nearest-neighbor search
tree = cKDTree(ecospace_df[["latitude", "longitude"]].values)

# Query nearest grid cell for each sample
coords = combined_df[["latitude", "longitude"]].values
distances, indices = tree.query(coords, k=1)

# Append ecospace match info
combined_df["ewe_row"] = ecospace_df.iloc[indices]["EWE_row"].values
combined_df["ewe_col"] = ecospace_df.iloc[indices]["EWE_col"].values
combined_df["ewe_lat"] = ecospace_df.iloc[indices]["latitude"].values
combined_df["ewe_lon"] = ecospace_df.iloc[indices]["longitude"].values
combined_df["ewe_dist_km"] = distances * 111  # approx. km assuming 1° ≈ 111 km
