...
Purpose
-------
Given a single January ASC map (e.g., CHUM_SPAWN_PPROB_01.asc), create
year-specific copies for a specified range:
    CHUM_SPAWN_PPROB_01_1950.asc
    CHUM_SPAWN_PPROB_01_1951.asc
    ...
    CHUM_SPAWN_PPROB_01_2024.asc

Usage
-----
python repeat_january_map_by_year.py \
    --jan_file /path/to/CHUM_SPAWN_PPROB_01.asc \
...
DEF_SPECIES = "CHINOOK"
DEF_SPECIES_SHRT = "CK"
DEF_STAGE = "rt" # rt is returning, oe is ocean entry
DEF_FILE_PATTERN = DEF_SPECIES + "_" + DEF_SPECIES_SHRT + "_" + DEF_STAGE + "_{mm}.asc"
DEF_BASE_NAME = DEF_SPECIES + '_SPAWN_PPROB'

DEFAULT_SPAWN_P    = Path("C://Users//Greig//Sync//6. SSMSP Model//Model Greig//Data//1. Salmon//All Species Run Timing Wilson Peacock 2025//MODIFIED//ASC for Adult Run Timing//" + DEF_SPECIES + " SPWN//")
...
def resolve_month_file(input_dir: Path, pattern: str, month: int) -> Path:
    if "{mm}" not in pattern:
        raise ValueError("Pattern must include '{mm}', e.g., 'CHUM_SPAWN_PPROB_{mm}.asc'")
...
def main():
    ...
    ap.add_argument("--input_dir", default=DEFAULT_SPAWN_P, type=Path, help="Directory containing the 12 monthly inputs.")
    ap.add_argument("--pattern", default=DEF_FILE_PATTERN, type=str, help="Filename pattern with {mm}, e.g., 'CHUM_SPAWN_PPROB_{mm}.asc'")
    ap.add_argument("--basename", default=DEF_BASE_NAME, type=str, help="Base name for output files (e.g., 'CHUM_SPAWN_PPROB')")
    ...
    ap.add_argument("--overwrite", default=True, action="store_true", help="Overwrite existing files.")
    args = ap.parse_args()
